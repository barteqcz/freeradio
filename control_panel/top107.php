<?php
    ...
    if (isset($_GET['delete'])) {
        ...
        $deleteStmt = $connection->prepare("DELETE FROM top107 WHERE id = ?");
    ...
    if (isset($_POST['editId'])) {
        ...
        $updateStmt = $connection->prepare("UPDATE top107 SET artist = ?, title = ? WHERE id = ?");
        $updateStmt->bind_param("ssi", $updatedArtist, $updatedTitle, $editId);
        $updateStmt->execute();
        $updateStmt->close();
                    ...
                    <?php while ($row = $result->fetch_assoc()): ?>
                        <div class="setup-box-card" id="row-<?php echo $row['id']; ?>">
                            <div class="setup-box-card-content">
                                ...
                                <p><strong>Hlasy:</strong> <?php echo htmlspecialchars($row['votes']); ?></p>
                                <p><strong>Umělec:</strong> <?php echo htmlspecialchars($row['artist']); ?></p>
                                <p><strong>Název skladby:</strong> <?php echo htmlspecialchars($row['title']); ?></p>
                            </div>
